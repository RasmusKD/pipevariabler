...
const ChestComponent: React.FC<ChestComponentProps> = ({
  ...
  useEffect(() => {
    ...
    img.src = `${process.env.PUBLIC_URL}/assets/images/icons/${chest.icon === 'compass' || chest.icon === 'recovery_compass' ? compassImage : chest.icon === 'clock' ? clockImage : `${chest.icon}.png`}`;
  ...
  const IconComponent: React.FC<{ icon: string }> = ({ icon }) => {
    ...
    return (
        <div ref={compassRef} className="cursor-pointer flex items-center justify-center" onClick={() => selectIcon(icon)}>
          <img
              src={`${process.env.PUBLIC_URL}/assets/images/icons/${icon === 'compass' || icon === 'recovery_compass' ? compassImage : icon === 'clock' ? clockImage : `${icon}.png`}`}
              alt={icon}
      ...
      >
        ...
        <div className="flex justify-between items-center gap-2">
          <div className="relative" ref={iconButtonRef}>
            <div ref={compassRef}>
              <img
                  src={`${process.env.PUBLIC_URL}/assets/images/icons/${chest.icon === 'compass' || chest.icon === 'recovery_compass' ? compassImage : chest.icon === 'clock' ? clockImage : `${chest.icon}.png`}`}
                  alt="icon"
